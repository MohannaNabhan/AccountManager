...
document.getElementById('maximize-btn').addEventListener('click', e=>{
    if(document.getElementById('maximize-btn').innerHTML == '<i style="margin-top: 8px;" class="fal fa-rectangle-landscape"></i>' ){
       ipcRenderer.send("event:window","maximize")
       document.getElementById('maximize-btn').innerHTML = `<i style="margin-top: 8px; transform: rotate(180deg);" class="far fa-clone"></i>`;
   }else{
       ipcRenderer.send("event:window","unmaximize")
       document.getElementById('maximize-btn').innerHTML = `<i style="margin-top: 8px;" class="fal fa-rectangle-landscape"></i>`;
   }
})
...
function notification(type,data) {
    ...
    if(type == "Alert:ERROR"){
        alert_.innerHTML = `<i class="fas fa-times-circle"></i>
        <h1>${data}</1h>`;
        alert_.style.background = "#A5104CFF";
    }else if(type == "Alert:GOOD"){
        alert_.innerHTML = `<i class="fas fa-badge-check"></i>
        <h1>${data}</1h>`;
        alert_.style.background = "#379100FF";
    }else if(type == "Alert:WARNING"){
        alert_.innerHTML = `<i class="fas fa-exclamation-triangle"></i>
        <h1>${data}</1h>`;
...
function window_events(title,data) {
    document.getElementById('window-info-back-full').classList.add('visible-window-info');
    document.getElementById('window-info-back').classList.add('visible-window-info');
    document.querySelectorAll('.close-window').forEach(btn => {
        btn.addEventListener('click', e => {
            document.getElementById('window-info-back-full').classList.remove('visible-window-info');
            document.getElementById('window-info-back').classList.remove('visible-window-info');
        });
    })
    ...
    if(title == "Create New Category"){
        document.getElementById('window-info-body').innerHTML = `
            <div class="input-back">
                <i class="icon fal fa-signature"></i>
        ...
        document.getElementById('create-category').addEventListener('click', e=>{
            ...
            }else if(!e.target.parentElement.querySelector('#name').value.match(/^[0-9a-zA-Z ?=.]+$/)){
                notification('Alert:WARNING','Invalid Category Name')
            }else{
                for (let i = 0; i < list_category.length; i++) {
                    if(e.target.parentElement.querySelector('#name').value == list_category[i]){
                    ...
                    if(i > (list_category.length - 2) && !file_search_result){
                        document.getElementById('window-info-back-full').classList.remove('visible-window-info');
                        document.getElementById('window-info-back').classList.remove('visible-window-info');
                        exec(`mkdir "C:\\AccountManager\\Category\\${e.target.parentElement.querySelector('#name').value}"`);
        ...
        document.getElementById('window-info-body').innerHTML = `
            <div class="input-back">
            <i class="icon fal fa-money-check-edit"></i>
            <input type="text" id="name" placeholder="Category Name" value="${data.querySelector('h1').textContent}">
            </div>
        ...
        document.getElementById('edit-category').addEventListener('click', e=>{
            ...
            for (let i = 0; i < list_category.length; i++) {
                ...
                if(i > (list_category.length - 2) && !file_search_result){
                    if(!e.target.parentElement.querySelector('#name').value.match(/^[0-9a-zA-Z ?=.]+$/)){
                        notification('Alert:WARNING','Invalid Category Name')
                    }else{
                        fs.rename(path_db+'/Category/'+data.querySelector('h1').textContent, path_db+'/Category/'+e.target.parentElement.querySelector('#name').value, function (err) {
                        if (err){
        ...
        document.getElementById('delete-category').addEventListener('click', e =>{
            fs.rmdir(path_db+'/Category/'+data.querySelector('h1').textContent, { recursive: true }, (err) => {
                if (err){
        ...
        `;
        document.getElementById('generate-pass-account').addEventListener('click', e =>{
            e.target.parentElement.querySelector('#password').value = word_random(20);
        })
        document.querySelector('.icon-function').addEventListener('click', e =>{
            if( e.target.parentElement.querySelector('#password').type == "text"){
                ...
                document.querySelector('.icon-function').classList.remove('fa-eye-slash');
        ...
        document.getElementById('save-account').addEventListener('click', e=>{
            ...
            if(user == "" && service == ""){
                ...
                fs.writeFile(path_db+'/Category/'+category_load+'/'+word_random(7)+'.AccountManager', `${service}|/|${user}|/|${password}`, function(err) {
                    if(err) {
        ...
        fs.readFile(path_db+'/Category/'+category_load+'/'+data.querySelector('#file-name').textContent, 'utf8' , (err, data) => {
            if (!err) {
                document.getElementById('window-info-body').innerHTML = `
                    <div class="input-back">
                    <i class="icon fas fa-concierge-bell"></i>
                    <input type="text" disabled="disabled" value="${data.split('|/|')[0]}">
                    </div>
                    <div style="margin-top: 50px;" class="input-back">
                    <i class="icon fas fa-user"></i>
                    <input type="text" id="email" disabled="disabled" value="${data.split('|/|')[1]}">
                    <i class="icon-function fal fal fa-copy"></i>
                    </div>
                    <div style="margin-top: 100px;" class="input-back">
                    <i class="icon fas fa-key"></i>
                    <input type="password" id="password" disabled="disabled" value="${data.split('|/|')[2]}">
                    <i class="icon-function fal fal fa-copy"></i>
                    </div>
                ...
                document.querySelectorAll('.icon-function').forEach(btn => {
                    btn.addEventListener('click', e => {    
                        navigator.clipboard.writeText(e.target.parentElement.querySelector('input').value);
                    });
                })
                document.querySelector('#edit-account').addEventListener('click', e =>{
        ...
        let file_name = data;
        fs.readFile(path_db+'/Category/'+category_load+'/'+data, 'utf8' , (err, data) => {
            if (!err) {
                document.getElementById('window-info-body').innerHTML = `
                <div class="input-back">
                    <i class="icon fas fa-concierge-bell"></i>
                    <input type="text" id="service" placeholder="Service Name" value="${data.split('|/|')[0]}">
                </div>
                <div style="margin-top: 50px;" class="input-back">
                    <i class="icon fas fa-user"></i>
                    <input type="text" id="email" placeholder="Email or Username" value="${data.split('|/|')[1]}">
                </div>
                <div style="margin-top: 100px;" class="input-back">
                    <i class="icon fas fa-key"></i>
                    <input type="password" id="password" placeholder="Password" value="${data.split('|/|')[2]}">
                    <i class="icon-function fal fa-eye-slash"></i>
                </div>
                <button style="top: 160px !important; right: 30px; background-color: transparent; width: 150px;" id="generate-pass-account" class="btn-event">Generate Password</button>
                ...
                document.querySelector('#save-account').addEventListener('click', e =>{
                    let service = e.target.parentElement.querySelector('#service').value;
                    let user = e.target.parentElement.querySelector('#email').value;
                    let password = e.target.parentElement.querySelector('#password').value;
                    if(user == "" && service == ""){
                        ...
                        fs.writeFile(path_db+'/Category/'+category_load+'/'+file_name, `${service}|/|${user}|/|${password}`, function(err) {
                            if(err) {
                                notification('Alert:ERROR',"Can't Create New Account")
                            }else{
                                document.getElementById('window-info-back-full').classList.remove('visible-window-info');
                ...
                document.querySelector('#delete-account').addEventListener('click', e =>{
                    fs.unlink(path_db+'/Category/'+category_load+'/'+file_name, (err) => {
                        if (err){
        ...
        fs.readdir(path_db + '/Category/'+category, function (err, files) {
            if (!err) {
                ...
                for (i = 0; i < files.length; i++) {
                    if(files[i].charAt(files[i].length-15)+files[i].charAt(files[i].length-14)+files[i].charAt(files[i].length-7) == ".AM"){
                        let file_name = files[i];
                        fs.readFile(path_db +'/Category/'+category+'/'+files[i], 'utf8' , (err, data) => {
                            if (!err) {
                                document.getElementById('account-information-back').innerHTML +=`
                                   <button class="account-information-item account-view">
                                       ...
                                       <h2>Email: **********************</h2>
                                       <h3>Password: ***********</h3>
                                       <i class="icon-2 fal fa-angle-right"></i>
                                   </button>
                                `; 
                                file_load++;
                            }
                ...
                setTimeout(() => {
                    if(file_load > (files.length - 2)){
                        ...
                        `;    
                        document.querySelector('.account-add').addEventListener('click',e =>{
                        ...
                        document.querySelectorAll('.account-view').forEach(btn => {
                            btn.addEventListener('click', e => {
                                ...
                                }else if(e.target.parentElement.classList.contains('account-view')){
...
function draw_category() {
    fs.readdir(path_db + '/Category', function (err, files) {
        if (!err) {
            document.getElementById('number-category').innerHTML = `${files.length > 8 ? 8 : files.length}/8`;
            ...
            if(files.length < 1){
                ...
                list_category = files;
                files.sort();
                let total_files = files.length > 8 ? 8 : files.length; 
                for (i = 0; i < total_files; i++) {
                    if(files[i].match(/^[0-9a-zA-Z ?=.]+$/)){
                        document.getElementById('category-list-item-back').innerHTML +=`
                            <div class="view-category category-list-item">
                                <h1>${files[i]}</h1> 
                                <i class="edit-category fal fa-pencil-alt"></i>
                            </div>
                        `;
                    }else{
                        console.error('Invalid File: '+files[i])
                    }
                    if(files.length < 8 && i > (files.length - 2)){
                        document.getElementById('category-list-item-back').innerHTML +=`
                            <div class="add-category category-list-item">
                                <h1>+</h1>
                            </div>
